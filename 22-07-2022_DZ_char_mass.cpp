...
int main()
{
	...
	while (iter1 != s1.end()-1 )    // пока не дойдем до конца
	{
		...
		{
			if (s1[inds1 + 1] == s2[inds2 + 1]) //сравниваем начальный элемент итерарора 2 с текущим элементом итератора 1
			{
				if (s1[inds1 + 2] == s2[inds2 + 2]) //сравниваем начальный элемент итерарора 2 с текущим элементом итератора 1
				{
					vec1 = true;
					inds2 = 0;
				}
